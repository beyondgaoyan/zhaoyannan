...
class plugin_mo_weibo_dzx {
			...
			if($this->scroll==2){
				while($value=DB::fetch($query)){
					...
					if(!$this->bgroup || !in_array($value[groupid],$this->bgroup)){
						$wbcol_list[$count] .= '<li><a href="home.php?mod=space&uid='.$value[uid].'" target="_blank" class="avatar z">'.avatar($value[uid],'small').'</a><div><p><a href="home.php?mod=space&uid='.$value[uid].'" target="_blank" c="1">'.$value[username].'</a>: <a href="home.php?mod=space&do=doing&view=all&doid='.$value[doid].'">'.strip_tags($value['message'], '<b><font><img>').'</a></p><p><em class="xg1">'.dgmdate($value['dateline'],'u').'&nbsp;<a href="plugin.php?id=mo_weibo_dzx&mod=reply&doid='.$value[doid].'" onclick="showWindow(\'mo_weibo_dzx\',this.href)">['.lang('plugin/mo_weibo_dzx', 'reply').']['.$value['replynum'].']</a></em></p></div></li>';
						$count++;
					}
				}
				...
				$wbcontent .= '<script type="text/javascript">'.$mo_script.'</script></div>';
			}else{
				...
				while($value=DB::fetch($query)){
					...
					if(!$this->bgroup || !in_array($value[groupid],$this->bgroup)){
						$wbcontent .= '<ol class="wbc"><a href="home.php?mod=space&uid='.$value[uid].'" target="_blank" class="avatar z">'.avatar($value[uid],'small').'</a><div><p><a href="home.php?mod=space&uid='.$value[uid].'" target="_blank" c="1">'.$value[username].'</a>: <a href="home.php?mod=space&do=doing&view=all&doid='.$value[doid].'">'.strip_tags($value['message'], '<b><font><img>').'</a></p><p><em class="xg1">'.dgmdate($value['dateline'],'u').'&nbsp;<a href="plugin.php?id=mo_weibo_dzx&mod=reply&doid='.$value[doid].'" onclick="showWindow(\'mo_weibo_dzx\',this.href)">['.lang('plugin/mo_weibo_dzx', 'reply').']['.$value['replynum'].']</a></em></p></div></ol>';
						$count++;
					}
				}
				$wbcontent .="</li></ul><script type=\"text/javascript\">mo_weibo('mo_wb','mo_wbc','".$this->wbspeed."')</script></div>";
			}
	...
	function global_footer() {
		...
		if($_G['cache']['plugin']['mo_weibo_dzx']['mo_weibo_popup']>0 && $_COOKIE['mo_weibo_dzx']!=1 && $_G[uid] && !in_array($_G[uid],$buser) && !in_array($_G[groupid],$bgroup)){
			...
			if($nowshort>$lastwb){
				return '<style type="text/css">.momoodfm{margin:15px;} .momoodfm textarea{width:310px!important; padding:5px; height:50px;} .momoodfm .moodfm_f{padding:5px; line-height:24px; vertical-align:middle;} .facel{width:260px; top:0; padding:0 6px 6px 0;} .facel img{margin:6px 0 0 6px;}.moodfm_btn{padding-left: 5px; background: url(static/image/common/mood_input_btn.png) no-repeat 5px 0;} .moodfm_btn button {width: 58px; height: 58px; cursor: pointer; opacity: 0; filter: alpha(opacity=0);}</style><script type="text/javascript">showWindow(\'mo_weibo_dzx\',\'plugin.php?id=mo_weibo_dzx&mod=doing\');</script>';
			}
		}
	}
}
