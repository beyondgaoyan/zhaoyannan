...
class table_aljbd_consume extends discuz_table{
	...
	public function counts_by_subtype($type){
		return DB::fetch_all('select * from %t where subtype=%d',array($this->_table,$type));
	}
	//获取子类商家数 by gaoyan
	public function count_by_subtype_gy(){
		return DB::fetch_all('select subtype,count(*) num from %t group by subtype',array($this->_table));
	}
	public function find_by_like($like){
	if($like!=''){
		$query="select * from pre_aljbd_consume where subject like '%".$like."%'";
