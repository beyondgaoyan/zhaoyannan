 ...
 *
 *      $Id: common.js 5773 2011-11-07 06:14:07Z menglingmin $
 */
...
function brand_search(form){
	...
	if ("shop" == mod) {
		form.action = "plugin.php?id=aljbd&act=search&mod=shop";
	} else if ("consume" == mod) {
		form.action = "plugin.php?id=aljbd&act=search";
	} else if ("goods" == mod) {
		form.action = "plugin.php?id=aljbd&act=search&mod=goods";
 ...
 * 新增多分类关联
 */
function createmultiselect(select_id, select_name, select_content, select_parent, select_init_val, multiple) {
	multiple=multiple?' multiple size="10"':'';
	var multipleadd=multiple?'s[]':'';
	var selector = 1;
	...
	function creat(id) {
		...
		if (newinnercontent != '') {
			$("#" + id + "").after('<select'+multiple+'><option value="-1">' + select_init_val + '</option>' + newinnercontent + '</select>');
			$("#" + id + "").removeAttr("name");
			$("#" + id + " + select").attr('name', select_name+multipleadd);
			$("#" + id + " + select").attr('id', 'selector' + select_name + '_' + selector);
			$("#" + id + " + select").bind('change', function() {
				creat(this.id);
			});
		} else {
			$("#" + id).attr('name', select_name+multipleadd);
		}
