...
if($_GET['lj']=='goodstype'){
	...
	if($act == 'del') {
		...
		cpmsg(lang('plugin/aljbd','s1'), 'action=plugins&operation=config&identifier=aljbd&pmod=brandtype&lj=shangtype', 'succeed');	
	}
	...
	function del(id) {
		if(confirm('<?php echo lang('plugin/brand', 'del_confirm');?>')) {
			window.location = '<?php echo ADMINSCRIPT;?>?action=plugins&operation=config&identifier=aljbd&pmod=brandtype&lj=shangtype&act=del&tid='+id;
		} else {
			return false;
		}
	}
	...
	<?php
		showformheader('plugins&operation=config&do='.$_GET['do'].'&identifier=aljbd&pmod=brandtype&lj=shangtype');
		showtableheader('');
		showsubtitle(array(lang('plugin/aljbd','s2'),lang('plugin/aljbd','s3'),  lang('plugin/aljbd','s4')));
		...
		cpmsg(lang('plugin/aljbd','s9'), 'action=plugins&operation=config&do='.$_GET['do'].'&identifier=aljbd&pmod=brandtype&lj=shangtype', 'succeed');	
	}
}else if($_GET['lj']=='quantype'){
	$act = $_GET['act'];
	if($act == 'del') {
		$tid = intval($_GET['tid']);
		if($tid) {
			$upcid = C::t('#aljbd#aljbd_type_quan')->fetch_upid_by_id($tid);
			if($upcid) {
				$subid = C::t('#aljbd#aljbd_type_quan')->fetch_subid_by_id($upcid);
				$subarr = explode(",", $subid);
				foreach($subarr as $key=>$value) {
					if($value == $tid) {
						unset($subarr[$key]);
				...
				C::t('#aljbd#aljbd_type_quan')->update($upcid,array('subid'=>implode(",", $subarr)));
			}
			C::t('#aljbd#aljbd_type_quan')->delete($tid);
		}
		cpmsg(lang('plugin/aljbd','s1'), 'action=plugins&operation=config&identifier=aljbd&pmod=brandtype&lj=quantype', 'succeed');	
	}
	...
	function del(id) {
		if(confirm('<?php echo lang('plugin/brand', 'del_confirm');?>')) {
			window.location = '<?php echo ADMINSCRIPT;?>?action=plugins&operation=config&identifier=aljbd&pmod=brandtype&lj=quantype&act=del&tid='+id;
		} else {
			return false;
		}
	}
	...
	<?php
		showformheader('plugins&operation=config&do='.$_GET['do'].'&identifier=aljbd&pmod=brandtype&lj=quantype');
		showtableheader('');
		showsubtitle(array(lang('plugin/aljbd','s2'),lang('plugin/aljbd','s3'),  lang('plugin/aljbd','s4')));

		$brandtype = C::t('#aljbd#aljbd_type_quan')->fetch_all_by_upid(0);
		foreach($brandtype as $key=>$value){

			$bt = C::t('#aljbd#aljbd_type_quan')->fetch_all_by_upid($key);
			foreach($bt as $k=>$v){
				$brandtype[$key]['subtype'][$k] = $v;
			}
		}
		...
		if(is_array($order)) {
			foreach($order as $id=>$value) {
				...
				C::t('#aljbd#aljbd_type_quan')->update($id,array('displayorder'=>$value,'subject'=>$name[$id]));
			}
		}
		...
		if(is_array($newcat)) {
			foreach($newcat as $key=>$name) {
				...
				$cid=C::t('#aljbd#aljbd_type_quan')->insert(array('upid' => '0', 'subject' => $name, 'displayorder' => $newcatorder[$key]),1);
			}
		}

		if(is_array($newsubcat)) {
			foreach($newsubcat as $cid=>$subcat) {
				$sub=C::t('#aljbd#aljbd_type_quan')->fetch($cid);
				$subtype =$sub['subid'];
				foreach($subcat as $key=>$name) {
					$subid=C::t('#aljbd#aljbd_type_quan')->insert(array('upid' => $cid, 'subject' => $name, 'displayorder' => $newsuborder[$cid][$key]),1);
					$subtype .= $subtype ? ','.$subid : $subid;
				}
				C::t('#aljbd#aljbd_type_quan')->update($cid,array('subid'=>$subtype));
			}
		}

		cpmsg(lang('plugin/aljbd','s9'), 'action=plugins&operation=config&do='.$_GET['do'].'&identifier=aljbd&pmod=brandtype&lj=quantype', 'succeed');	
	}
}else{
	$act = $_GET['act'];
	if($act == 'del') {
